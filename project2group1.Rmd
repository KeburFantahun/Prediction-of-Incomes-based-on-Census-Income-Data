...
#First up, we will use a different sampling technique on the data due to the imbalance between >50k and <50K
```{r}
#Check the balance of the data
#Plot Sex vs income
p<-ggplot(data=incomeDataUSE, aes(x=income, y="count", fill=income)) +
  geom_bar(stat="identity")+ggtitle("Occupation Distribution")+ylab("Count")+xlab("Categories")
p
#Clearly most of the data <=50k
#Lets make a table to see the split
IncomeDf <- data.frame(table(incomeDataUSE$income))
names(eduVsIncomeDf) <- c('income', 'count')
#test
IncomeDf

incomeSum <- sum(IncomeDf$Freq)
f1 <- 0.75902763
f2 <- 1-f1

#Data is ~75/25 split, heavily imbalanced toward those who <=50k/year
